...
int remotePort;
String datoExtraido ="";
String control_frecuencia_respiratoria="";
WiFiUDP udp;
...
String extraerDato(char *datoCompleto)
{
  ...
  if (inicio != -1 && fin != -1)
  {
    ...
    if (datoString.startsWith("frecuencia_respiratoria:"))
    {
      // Encontrar la posición del primer ':'
    ...
    {
      return "No se encontró la palabra clave 'frecuencia_respiratoria:'";
    }
  }
  else
  {
...
void mostrar_lcd(int mostrar,String dato){
  ...
  case 1:
    ...
    lcd.setCursor(0, 1);
    lcd.print(dato);
    lcd.print(" res.. de F.R.");
    break;
  case 2:
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("C. Frecuencia R.:");
    lcd.setCursor(0, 1);
    lcd.print(dato);
    lcd.print(" bpm");
    break;
...
void setup()
{
  ...
  IPAddress localIP(192, 168, 1, 102); // Cambia esto según tu configuración
...
void loop()
{
  ...
  if (packetSize)
  {
    ...
    control_frecuencia_respiratoria = datoExtraido;
    Serial.print("badera mensaje:");
...
if(bandera_mensaje == 1){
  ...
  udp.print(">respuesta_frecuencia_respiratoria:"+control_frecuencia_respiratoria+"<");
  udp.endPacket();
  mostrar_lcd(bandera_mensaje,datoExtraido);
  }
...
if(bandera_mensaje == 2){
  mostrar_lcd(3,control_frecuencia_respiratoria);
