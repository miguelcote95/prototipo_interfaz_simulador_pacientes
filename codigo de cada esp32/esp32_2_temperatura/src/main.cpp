...
String extraerDato(char *datoCompleto)
{
  ...
  if (inicio != -1 && fin != -1)
  {
    ...
    if (datoString.startsWith("temperatura:"))
    {
      // Encontrar la posición del primer ':'
    ...
    {
      return "No se encontró la palabra clave 'temperatura:'";
    }
  }
  else
  {
...
void setup()
{
  ...
  IPAddress localIP(192, 168, 1, 101); // Cambia esto según tu configuración
...
void loop()
{
  ...
  if (packetSize)
  {
    ...
    String datoExtraido = extraerDato(incomingPacket);
    Serial.println("Control de temperatura: " + datoExtraido);
    mostrar_lcd(2,datoExtraido);
    control_temperatura = datoExtraido;
...
if(bandera_mensaje == 1){
  ...
  udp.print(">respuesta_temperatura:"+control_temperatura+"<");
  udp.endPacket();
  mostrar_lcd(bandera_mensaje,datoExtraido);
  }
...
if(bandera_mensaje == 2){
  mostrar_lcd(3,control_temperatura);
